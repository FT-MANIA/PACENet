...
class TimesBlock(nn.Module):
    def __init__(self, config):
        ...
        self.k = config.get('timesnet_top_k', 5)

        d_model = config.get('timesnet_d_model', 64)
        d_ff = config.get('timesnet_d_ff', d_model * 4)
        num_kernels = config.get('timesnet_num_kernels', 3)
...
class TimesNet(nn.Module):
    def __init__(self, config):
        ...
        head_dim = config.get('timesnet_head_dim', 128)
        self.projection = nn.Sequential(
            nn.Linear(self.d_model * 2, head_dim),
            nn.LayerNorm(head_dim),
            nn.GELU(),
            nn.Dropout(config.get('timesnet_head_dropout', 0.3)),
    ...
    def forward(self, x):
        ...
        avg_feat = output.mean(dim=1)
        max_feat = output.max(dim=1)[0]
        feat = torch.cat([avg_feat, max_feat], dim=-1)

        output = self.projection(feat)
